...
import { getFieldsToSort } from './getFieldsToSort';
import { query, queryData, queryRepeatKey } from './query';
import { currentDay, monthsNames, previousDay, years, dateIsValid } from './date';
import { formatNumber } from './formatters/numberFormatters';
import { formatDate } from './formatters/dateFormatters';
import { getRangeLabels, isRangeFilter } from './range';
...
const getColumnToSort = (text: string[], sortDesc = true): string => {
  const result = text?.length
    ? text[0].split('.').map((item, index) => index ? item.charAt(0).toUpperCase() + item.substr(1) : item).join('')
    : '';
